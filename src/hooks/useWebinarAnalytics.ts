...
export const useWebinarAnalytics = (filters: WebinarAnalyticsFilters) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        // Fetch webinars with participants - handle potential query errors
        ...
        if (filters.status) {
          webinarsQuery.eq('status', filters.status);
        }
        ...
        // Type the webinars data properly, handling potential query errors
        const typedWebinars = webinars.map(webinar => ({
          ...webinar,
          zoom_participants: Array.isArray(webinar.zoom_participants) ? webinar.zoom_participants : []
        })) as WebinarData[];
        ...
        const webinarEngagements = await Promise.all(
          typedWebinars.map(async (webinar) => {
            try {
              ...
              console.error(`Error calculating engagement for webinar ${webinar.id}:`, error);
              return { webinar, engagement: null };
            }
          })
        );
        ...
        console.error('Error in webinar analytics query:', error);
        throw error;
      }
    },
...
async function prepareGeographicData(webinars: WebinarData[]): Promise<ChartData['geographicData']> {
  ...
  try {
    ...
    console.error('Error preparing geographic data:', error);
    return [];
  }
}
