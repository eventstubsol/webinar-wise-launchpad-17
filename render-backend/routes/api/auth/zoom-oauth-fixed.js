...
// Zoom OAuth configuration with FIXED redirect URI
const ZOOM_CLIENT_ID = process.env.ZOOM_OAUTH_CLIENT_ID;
const ZOOM_CLIENT_SECRET = process.env.ZOOM_OAUTH_CLIENT_SECRET;

// IMPORTANT: Hardcode the redirect URI to match EXACTLY what's in your Zoom app
const ZOOM_REDIRECT_URI = 'http://localhost:3001/api/auth/zoom/callback';
// For production, use: 'https://webinar-wise-launchpad-17.onrender.com/api/auth/zoom/callback'

console.log('=== ZOOM OAUTH CONFIGURATION (FIXED) ===');
...
console.log('Redirect URI (HARDCODED):', ZOOM_REDIRECT_URI);
...
router.get('/zoom/authorize', async (req, res) => {
  try {
    ...
    // Build Zoom OAuth URL with encoded redirect URI
    const params = new URLSearchParams({
      response_type: 'code',
      client_id: ZOOM_CLIENT_ID,
      redirect_uri: ZOOM_REDIRECT_URI, // Using hardcoded value
      state: state,
      scope: ZOOM_SCOPES
    });
    ...
    // Let's also log the encoded version to debug
    console.log('Encoded Redirect URI:', encodeURI(ZOOM_REDIRECT_URI));
    ...
    res.json({ 
      ...
      debug: {
        ...
        redirectUri: ZOOM_REDIRECT_URI,
        encodedRedirectUri: encodeURI(ZOOM_REDIRECT_URI),
...
router.get('/zoom/callback', async (req, res) => {
  try {
    ...
    // Exchange code for access token with SAME redirect URI
    const tokenUrl = 'https://zoom.us/oauth/token';
    const tokenParams = new URLSearchParams({
      grant_type: 'authorization_code',
      code: code,
      redirect_uri: ZOOM_REDIRECT_URI // Using same hardcoded value
    });
