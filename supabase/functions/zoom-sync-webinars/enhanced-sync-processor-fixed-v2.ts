...
console.log('📦 Enhanced sync processor FIXED with participant data fetching loaded successfully');
    ...
    for (let batchStart = 0; batchStart < webinarsToProcess.length; batchStart += BATCH_SIZE) {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        try {
          ...
          // Fetch additional data for past webinars
          let participantCount = 0;
          let registrantCount = 0;
          
          // Determine if webinar is past based on status and start time
          const isPastWebinar = webinarDetails.status === 'finished' || 
                              webinarDetails.status === 'ended' ||
                              (webinarDetails.start_time && new Date(webinarDetails.start_time) < new Date());
          
          if (isPastWebinar) {
            console.log(`📊 Fetching participant data for past webinar ${webinar.id}`);
            
            // Fetch participants
            try {
              const participants = await client.getWebinarParticipants(webinar.id);
              participantCount = participants.length;
              console.log(`✅ Found ${participantCount} participants for webinar ${webinar.id}`);
            } catch (error) {
              console.error(`⚠️ Could not fetch participants for webinar ${webinar.id}: ${error.message}`);
            }
            ...
            try {
              ...
              console.log(`✅ Found ${registrantCount} registrants for webinar ${webinar.id}`);
            } catch (error) {
              console.error(`⚠️ Could not fetch registrants for webinar ${webinar.id}: ${error.message}`);
            }
          }
          
          // Store webinar in database with fetched counts
          const storedWebinar = await storeWebinarEnhancedWithCounts(
            supabase, 
            webinarDetails, 
            connection.id,
            participantCount,
...
// Enhanced store function that accepts participant and registrant counts
async function storeWebinarEnhancedWithCounts(
  supabase: any, 
  webinar: any, 
  connectionId: string,
  ...
  try {
    ...
    // Determine status based on webinar type
    let status = webinar.status;
    if (isPastWebinar) {
      // Map 'finished' to 'ended' for consistency
      status = (status === 'finished' || status === 'ended') ? 'ended' : status;
    }
    ...
    const webinarData: any = {
      ...
      // Attendance data - Use fetched counts if available
      total_registrants: registrantCount || webinar.registrants_count || webinar.total_registrants || 0,
      total_attendees: participantCount || webinar.total_attendees || webinar.participants_count || 0,
    ...
    console.log(`📊 Storing webinar ${webinar.id} with metrics - Registrants: ${webinarData.total_registrants}, Attendees: ${webinarData.total_attendees}, Absentees: ${webinarData.total_absentees}`);
    
    // Use 'select' to get the stored data back
