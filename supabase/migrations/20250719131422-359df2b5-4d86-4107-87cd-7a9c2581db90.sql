...
-- Phase 1: Fix Database Schema - Add missing unique constraints and indexes

-- First, let's ensure we have the missing participant_id column in zoom_participants
DO $$ 
BEGIN
...
-- For cases where both are null, generate a unique participant_id from available data
UPDATE zoom_participants 
SET participant_id = COALESCE(participant_email, participant_name, 'unknown_' || id::text)
